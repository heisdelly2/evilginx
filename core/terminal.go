...
func (t *Terminal) DoWork() {
	...
	for !do_quit {
		...
		case "checktelegram":
			cmd_ok = true
			err := t.handleCheckTelegram(args[1:])
			if err != nil {
				log.Error("checktelegram: %v", err)
			}
		case "test-certs":
			cmd_ok = true
			t.manageCertificates(true)
...
func (t *Terminal) handleCheckTelegram(args []string) error {
	...
	if botToken == "" || chatID == "" {
		...
		log.Success("Telegram configuration is complete. Notifications are enabled.")
	}

	return nil
}

// Add this to the commands map in the init() function:
