...
func printUpdateName() {
	...
	txt := nameClr.Sprintf("               - --  Antibot Edition  -- -")
...
func printOneliner1() {
	...
	txt := textClr.Sprintf("      moded by Op Dante (") + handleClr.Sprintf("@Cyber_redneak") + textClr.Sprintf(")") + spc + textClr.Sprintf("version ") + versionClr.Sprintf("%s", VERSION)
	fmt.Fprintf(color.Output, "%s", txt)
}
